...
for epoch in range(EPOCHS):
    ...
    if(epoch+1) %20  == 0 :
         print(f"Epoch {epoch+1:3d}/{EPOCHS} | "  #来个3d对齐下
...
y_sample = y_train_onehot[0:1]

Z1_sample,A1_sample,Z2_sample,A2_sample,Z3_sample,A3_sample = forward(
    X_sample,W1_init,b1_init,W2_init,b2_init,W3_init,b3_init
)

dW1_sample,db1_sample,dW2_sample,db2_sample,dW3_sample,db3_sample = backward(
    X_sample,y_sample,Z1_sample,A1_sample,Z2_sample,A2_sample,Z3_sample,A3_sample,W1_init,W2_init,W3_init
)

# 假设你已经计算了梯度
# dW1, db1, dW2, db2, dW3, db3 都是numpy数组

# 打印第一个样本的梯度
print("\n=== 第一个样本的参数梯度 ===")

# W1的梯度 (4,16) - 第一层到第二层的权重梯度
print("dW1 (4,16) 前2行前2列：")
print(dW1_sample[:2, :2])  # 显示前2行2列

# b1的梯度 (1,16) - 第一层到第二层的偏置梯度
print("db1 (1,16) 前5个值：")
print(db1_sample[0, :5])   # 显示前5个值

# W2的梯度 (16,8) - 第二层到第三层的权重梯度
print("dW2 (16,8) 前2行前2列：")
print(dW2_sample[:2, :2])  # 显示前2行2列

# b2的梯度 (1,8) - 第二层到第三层的偏置梯度
print("db2 (1,8) 前5个值：")
print(db2_sample[0, :5])   # 显示前5个值

# W3的梯度 (8,3) - 第三层到输出层的权重梯度
print("dW3 (8,3) 前2行：")
print(dW3_sample[:2, :])   # 显示前2行

# b3的梯度 (1,3) - 第三层到输出层的偏置梯度
print("db3 (1,3)：")
print(db3_sample)          # 显示所有值


#这个是两个同时开 如果没有这个 需要关闭一个 然后才会显示 这是matplotlib的机制
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 4))

# 左边：损失曲线
ax1.plot(range(1, EPOCHS+1), train_loss_history, label="Train Loss", color="blue", linewidth=1.5)
ax1.plot(range(1, EPOCHS+1), val_loss_history, label="Val Loss", color="orange", linewidth=1.5)
ax1.set_xlabel("Epoch", fontsize=12)
ax1.set_ylabel("Cross-Entropy Loss", fontsize=12)
ax1.set_title("Training & Validation Loss Curve", fontsize=14)
ax1.legend(fontsize=10)
ax1.grid(alpha=0.3)

# 右边：准确率曲线
ax2.plot(range(1, EPOCHS+1), train_acc_history, label="Train Acc", color="blue", linewidth=1.5)
ax2.plot(range(1, EPOCHS+1), val_acc_history, label="Val Acc", color="orange", linewidth=1.5)
ax2.set_xlabel("Epoch", fontsize=12)
ax2.set_ylabel("Accuracy", fontsize=12)
ax2.set_title("Training & Validation Accuracy Curve", fontsize=14)
...
#测试集
Z1_test, A1_test, Z2_test, A2_test, Z3_test, A3_test = forward(
    X_test, W1, b1, W2, b2, W3, b3
)

test_loss = compute_cross_entropy(A3_test, y_test_onehot)
test_acc = compute_accuracy(A3_test, y_test_onehot)
